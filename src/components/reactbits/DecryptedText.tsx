...
export interface DecryptedTextProps extends Omit<HTMLMotionProps<"span">, "style"> {
...
export default function DecryptedText({
  ...
  const [isDecrypted, setIsDecrypted] = useState(
    animateOn !== "click" && !startEncrypted
  );
  ...
  /* Controlled Trigger */
  useEffect(() => {
    if (animateOn === "controlled" && isTriggered && !hasAnimated) {
      if (delay > 0) {
        delayTimeoutRef.current = setTimeout(() => {
          triggerDecrypt();
          setHasAnimated(true);
  ...
  }, [animateOn, isTriggered, hasAnimated, triggerDecrypt, delay]);

  useEffect(() => {
    if (animateOn === "click" || startEncrypted) {
      encryptInstantly();
    } else {
      setDisplayText(text);
      setIsDecrypted(true);
    ...
    setRevealedIndices(new Set());
    setDirection("forward");
  }, [animateOn, text, encryptInstantly, startEncrypted]);
  ...
  return (
    <motion.span
      ...
      ref={containerRef}
      style={{ ...styles.wrapper, ...(props.style as any) }}
      {...animateProps}
      {...props}
    >
